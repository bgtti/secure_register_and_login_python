...
def get_client_ip(request: object) -> str | None:
    """
    Gets the client IP from a request header and returns the IP as string (if valid) or None (if the address could not be verified).

    :param request (object): request headers
    :returns (str | None): IP or None

    **Example usage:**
    ```
    from flask import request
    # ...inside route or function:
