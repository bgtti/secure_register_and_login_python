...
# DB models
from app.models.log_security import LogSecurity
...
def svc_add_log_security(level: str, event: SecurityEvent, activity: str, message: str, more_info: str, ip: str, user_agent: str, user_id: int) -> None:
    """
    Adds a log to LogSecurity db table.

    :param level (str): a key in LOG_LEVEL dictionary (`app/constants/log_levels.py`)
    :param event (SecurityEvent): enum in SecurityEvent (`app/constants/log_event_security.py`)
    ...
    try: 
        ...
        "WARNING",
        SecurityEvent.ACCOUNT_BLOCKED_BY_SYSTEM,
        "account creation",
        "Multiple failed login attempts. User temporarily blocked by the system.",
        "Login route: 11 failed attempts counted.",
        "192.168.1.100",
        ...
        db.session.rollback()
        logging.error(f"LogSecurity creation failed. Log event: {event}, user id: {user_id}  Error: {e}")
    ```
    """
    if not level in LOG_LEVEL:
        raise ValueError("Level must be a member of LOG_LEVEL - dictionary in 'constants' package.")
    if not isinstance(event, SecurityEvent):
        raise ValueError("Event must be a member of SecurityEvent - enum class in 'constants' package.")
    ...
    try:
        ...
        db.session.rollback()
        logging.error(f"LogSecurity creation failed. Log activity: {activity_lc}, level: {level}  Error: {e}")
    return



def svc_user_security_log_table(user_id: int, page_nr: int, items_per_page: int = 25, internal_use: bool = False) -> dict | None:
    """
    Serializes the user's security log table paginated. Will be ordered descending by created_at date.
    ...
    ```python
    {
        ...
        "logs": [
            ...
            "ip_address": "192.168.1.1", # only if internal_use == True
            "geo_location": "USA, New York", # only if internal_use == True
    ...
    if not isinstance(user_id, int) or user_id < 1:
        logging.error("svc_user_security_log_table received invalid user_id.")
        return None
    if page_nr < 1 or items_per_page < 1 or items_per_page > 100:
        logging.error("svc_user_security_log_table received invalid page_nr or items_per_page.")
        return None
    
    # Get logs
    try:
        logs = LogSecurity.query.filter_by(user_id=user_id).order_by(LogSecurity.created_at.desc()).paginate(page=page_nr, per_page=items_per_page, error_out=False)
        if not logs.items:
            return None
    except Exception as e:
